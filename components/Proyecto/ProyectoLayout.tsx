    ...
    <div className='relative'>
        {
            ...
            <div>
                ...
                <div className='h-screen w-screen flex lg:flex-row flex-col-reverse justify-between lg:items-center items-end relative'>
                    <Image src={proyecto.desarrollo_alianza.portada.url} alt={`Imagen principal proyecto: ${proyecto.nombre}`} fill sizes='100vw' className='duration-700 object-cover z-10' />
                    ...
                    <div className='flex max-w-[1450px] h-screen'>
                        <div className='flex flex-col justify-between lg:min-w-[40vw] min-w-screen h-full relative z-20  '>
                            ...
                            <div className='bg-white flex w-screen lg:w-auto'>
                                <div style={{backgroundColor: `${proyecto.color_secundario}`}} className={`${proyecto.desarrollo_alianza?.nombre ? "bg-primary-gray" : ""} p-2 w-24 h-40 flex justify-center items-center text-xs tracking-[3px]`}>
                                    <h5 style={{color: `${proyecto.texto_blanco.length === 0 ? '' : '#FFFFFF'}`}} className={`${proyecto.desarrollo_alianza?.nombre ? "text-white" : "#383838"} -rotate-90`}>descubrir</h5>
                                </div>
                                <div className='flex justify-center items-center w-full'>
                                        <h5 className='tracking-[2px] text-center px-2 text-black'><strong>{proyecto.desarrollo_alianza?.nombre}</strong>, {proyecto.desarrollo_alianza?.tipo_de_proyecto}</h5>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className=''>
                    <div className='flex flex-col md:max-w-[1200px] max-w-[85vw] mx-auto  py-12 gap-6'>
                        <h1 className='text-6xl pb-8 font-jakarta font-bold'>{proyecto.desarrollo_alianza?.nombre}</h1>
                        <div className='mb-8'>
                            <SwiperGallery images={proyecto.desarrollo_alianza?.galeria} />
                        </div>
                        <p className='text-2xl'><strong>Tipo de Proyecto:</strong> {proyecto.desarrollo_alianza?.tipo_de_proyecto}</p>
                        <p className='text-2xl'><strong>Fecha de Entrega:</strong> {proyecto.desarrollo_alianza?.fecha_de_entrega}</p>
                        <p className='text-2xl'><strong>Servicios/Amenities:</strong> {}</p>
                        <p className='text-2xl'><strong>Ubicación:</strong> {proyecto.desarrollo_alianza?.ubicacion}</p>
                        <Link
                            href={proyecto.desarrollo_alianza.ubicacion_mapa}
                            rel='noreferrer'
                            target='_blank'
                            className='cursor-pointer w-fit uppercase group relative bg-primary-gray tracking-[3px] text-xs text-white font-thin rounded-4xl overflow-hidden border
                        ...
                        <Link
                            ...
                            py-4 px-8 before:w-full before:h-full hover:before:translate-y-0 before:duration-500 before:ease-out before:absolute before:bg-green-300 before:left-0 before:top-0 before:rounded-4xl before:translate-y-full'>
                            
                            <p className='relative z-10 text-black'>Contactanos</p>
                            <Image
                                alt={'WhatsApp icon'}
            ...
            <div className={`relative`}>
                ...
                <div className='h-screen max-w-screen flex lg:flex-row flex-col-reverse justify-between lg:items-center items-end relative'>
                                ...
                                <div className='flex justify-center items-center w-full'>
                                    {
                                        ...
                                        <h5 className='tracking-[2px] text-center px-2 text-black'><strong>{proyecto.desarrollo_alianza?.nombre}</strong>, {proyecto.desarrollo_alianza?.tipo_de_proyecto}</h5>
                                        :
                                        <h5 className='tracking-[3px] text-sm text-center px-2'><strong>{proyecto.nombre}</strong>, {proyecto.titulo}</h5>
                                    }
                                </div>
                            </div>
                        </div>
                ...
                <div className='flex md:max-w-[1450px] mx-auto'>
                    ...
                    <div style={{backgroundColor: `${proyecto.color_primario}`}} className={`${proyecto.desarrollo_alianza?.nombre ? "bg-primary-gray" : ""} md:px-24 px-6 flex flex-col gap-2 md:gap-4`}>
